# Changelog

All notable changes to this project will be documented here. Format based on [Keep a Changelog](https://keepachangelog.com). Versioning follows MAJOR.MINOR.PATCH.

## [0.1.13] — 2026-06-02

### Added
- RPT-20260602T090000-SCP-1.00.md: Last weekly report before defence

### Changed
- TPL-LOG-SCP updated to v1.10 with TFG-specific fields
- TPL-ZN-SCP updated to v1.10 with TFG-specific fields
- TPL-RPT-SCP updated to v1.10 with QL21 data
- TPL-PROJ-SCP updated to v1.10 with QL21 data pre-filled
- D2 Documentation Framework complete

### Milestones
- 2026-06-21 TFG LaTeX (upcoming)

## [0.1.12] — 2026-05-19

### Added
- Pilot logs and outputs folder structure
- 01-literature/references/references.bib

### Changed
- README updated with final project status

## [0.1.11.2] — 2026-05-12

### Added
- LOG-20260512T090000-SCP-1.00.md: Survey data processing log
- Survey data processed with Python (Google Colab)
- Power BI dashboard created with QL21 theme
## [0.1.11.1] — 2026-05-11

### Added
- LOG-20260508T090000-SCP-1.00.md: Survey distribution log
- 113 valid responses collected

### Milestones
- Survey closed 2026-05-11
- Data exported for processing
## [0.1.11] — 2026-05-19

### Added
- TPL-LOG-SCP-1.00.md: Laboratory log adapted for QL21
- TPL-ZN-SCP-1.00.md: Zettel Note adapted for QL21
- TPL-RPT-SCP-1.00.md: Weekly report adapted for QL21
- TPL-PROJ-SCP-1.00.md: Project document adapted for QL21
- 04-outputs/D5-tfg-memory/README.md

## [0.1.10] — 2026-05-08

### Added
- D1-state-of-the-art.md closed as v1.0 Final
- Survey distribution begins (O1)
- README updated with dual workflow

## [0.1.9] — 2026-05-05

### Changed
- D1: Finding 5 and key decisions completed

## [0.1.8] — 2026-04-28

### Changed
- D1: Findings 3 and 4 completed

### Milestones
- Meeting with Esteve: confidential framework testing session

## [0.1.7] — 2026-04-20

### Changed
- D1: Findings 1 and 2 completed

## [0.1.6] — 2026-04-13

### Added
- D1-state-of-the-art.md initial structure
- Five key findings identified

## [0.1.5] — 2026-04-07

### Added
- SOW updated to v1.1 with O0 and O1 objectives
- sergi-casado-profile-1.1.yaml
- tfg-project-manifest-1.2.yaml
- tfg-branding-manifest-1.1.yaml

### Changed
- Project scope extended to include survey (O1)
- DaC philosophy formally integrated (O0)

### Milestones
- Meeting with Esteve: SOW v1.1 and manifests updated

## [0.1.4] — 2026-02-20

### Added
- 02-framework/templates/ created
- TPL-LOG.md: Laboratory log template
- TPL-ZN.md: Zettel Note template
- TPL-RPT.md: Weekly report template
- TPL-PROJ.md: Project initialisation template

### Milestones
- 2026-03-10 Meeting with Esteve: confidential framework testing session
- 2026-04-04 Meeting with Fran: templates review

## [0.1.3.1] — 2026-01-22

### Added
- 04-outputs/D5-tfg-memory/ folder structure created
- TFG memory writing begins

## [0.1.3] — 2026-01-15

### Added
- sergi-casado-profile-1.0.yaml
- tfg-project-manifest-1.1.yaml
- tfg-branding-manifest-1.0.yaml

### Milestones
- KOM milestone reached

## [0.1.2] — 2025-12-10

### Added
- SOW-TFG-2026-SCP-RACO-v0.9.md: Project scope formally defined
- Six specific objectives established

## [0.1.1] — 2025-11-14

### Added
- First literature notes on Write the Docs Berlin 2025
- Documentation as Code identified as core reference

## [0.1.0] — 2025-10-28

### Added
- Initial repository structure
- README.md with project overview
- CHANGELOG.md
- LICENSE.md
- Empty folder structure (00-governance to 05-journal)